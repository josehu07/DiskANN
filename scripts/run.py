...
def handle_build(dataset):
    ...
    options = [
        '--data_type', 'float', '--dist_fn', 'l2', '--data_path',
        learn_fbin_path, '--index_path_prefix', index_path_prefix, '-R', '32',
        '-L', '50', '-B', '0.003', '-M', '1'
    ]
...
def handle_query(dataset,
    ...
    options = [
        ...
        index_path_prefix, '--query_file', query_fbin_path, '--gt_file',
        gt_file_path, '-K',
        str(k_depth), '--result_path', res_path_prefix, '--num_nodes_to_cache',
        str(npts_to_cache), '-L'
    ]
    for l in list_sizes:
...
if __name__ == "__main__":
    ...
    parser_to_fbin = subparsers.add_parser(
        'to_fbin', help="generate input binary from sift fvecs")
    parser_to_fbin.add_argument(
        '--sift_base',
        help="input prefix, should be the prefix <this>_learn.fvecs",
        required=True)
    parser_to_fbin.add_argument(
        '--dataset',
        help="dataset name, will be the prefix <this>_learn.fbin",
        required=True)
    parser_to_fbin.add_argument(
        '--max_npts',
        help="#points to extract (if want smaller dataset)",
        type=int,
        default=0)

    parser_build = subparsers.add_parser(
        'build', help="build on-disk index from fbin file")
    parser_build.add_argument(
        '--dataset',
        help="dataset name, should be the prefix <this>_learn.fbin",
        required=True)

    parser_convert = subparsers.add_parser(
        'convert', help="convert disk index to zarr format tensors")
    ...
    parser_query = subparsers.add_parser(
        'query', help="run query (search) on index in various modes")
    parser_query.add_argument(
        '--dataset',
        help="dataset name, should be the prefix <this>_learn.fbin",
        required=True)
    parser_query.add_argument('--k_depth',
                              help="how many nearest neighbors to query",
                              type=int,
                              default=10)
    parser_query.add_argument('--npts_to_cache',
                              help="#points to cache ahead of time",
                              type=int,
                              default=10000)
    parser_query.add_argument('--use_ts',
                              action='store_true',
                              help="use tensorstore backend")
    parser_query.add_argument('--ts_async',
                              action='store_true',
                              help="use tensorstore in async mode")
    parser_query.add_argument('--list_sizes',
                              help="list of search list sizes",
                              metavar='L',
                              type=int,
                              nargs='+',
                              default=('10', '50', '100'))
